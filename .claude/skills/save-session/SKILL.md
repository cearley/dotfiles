...
natural rollover point — check its size after the update above (read_note, or `wc -l` on
its file under `~/.local/share/basic-memory/chezmoi/`). If it is at or over 200
lines, it's carrying more than "current state" and needs triage, not just editing.
...
Repeat until the note is at or under 200 lines. Confirm the final line count alongside
the note titles below.
