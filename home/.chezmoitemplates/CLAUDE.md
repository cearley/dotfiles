...
- `claude-tooling-guidance` - Body of the "Claude Code Tooling in the chezmoi Dotfiles Repo" guidance (everything after the frontmatter). Included by both `dot_claude/rules/claude-tooling.md.tmpl` (for the path-frontmatter auto-load on Read/Edit/Write) and `dot_local/bin/executable_claude-tooling-guard.tmpl` (baked into a quoted heredoc so the PreToolUse hook can inject the full guidance into `systemMessage`, not just point at the file, when a Bash command touches the same paths). Edit the partial, not either generated copy.
