...
- [ ] 1.1 Generate the age keypair (`age-keygen`)
- [ ] 1.2 Create a new KeePassXC entry (e.g. "age (dotfiles)") and store the private key as an attribute
- [ ] 1.3 Resolve the open design question on-disk key path (`~/.config/sops/age/keys.txt` vs. a repo-namespaced path) and document the decision
...
- [ ] 2.3 Create a bootstrap script in the 40-49 (Environment Setup) range — strictly before position 91 (`setup-cloudflare-tunnel`) — that fetches the private key via `keepassxcAttribute` and writes it to the path chosen in 1.3 with `0600` permissions, sourcing `scripts/shared-utils.sh` for messaging
- [ ] 2.4 Resolve the open design question on `run_once_` vs. `run_onchange_` semantics for this script and implement accordingly
- [ ] 2.5 Verify the new script's position number doesn't collide with existing scripts (`ls home/.chezmoiscripts/`)

## 3. SOPS Integration Plumbing

- [ ] 3.1 Create `.sops.yaml` at the repo root with the pilot's age public key as sole recipient, scoped via a `path_regex` matching only the two Cloudflare tunnel encrypted files
