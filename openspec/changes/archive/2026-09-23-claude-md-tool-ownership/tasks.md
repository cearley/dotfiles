...
- [x] 5.7 Mac Studio: pull and run `chezmoi apply` (script 44 removes the persona symlinks), run `omc setup` in each OMC persona, then move `~/.claude/CLAUDE.md` aside as in 5.6. Verify with `/context` in a named persona: exactly one `~/.claude-<name>/CLAUDE.md`, no `~/.claude/CLAUDE.md`, and `rules/global-preferences.md` present. Done on Mac Studio 2026-09-23, as reported by the user.
