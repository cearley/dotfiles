...
- [ ] 1.3 Archive `chezmoi-managed-plugin-marketplace` before archiving this change, so the main `claude-plugin-marketplace` spec contains its "chezmoi-managed and templatable" requirement that this change's delta builds on. Verify `grep -c "templatable" openspec/specs/claude-plugin-marketplace/spec.md` returns at least 1.
- [ ] 1.2 In a logged-in scratch persona (copy of `~/.claude-personal`, never the live dir), install a minimal local plugin whose `PreToolUse` hook (matcher `Read`) emits `additionalContext` once. Verify the context reaches the model, and that it fires inside a subagent. This closes the one spike question left untested.
...
- [ ] 3.2 Add `home/.chezmoitemplates/plugin-content-hash` and document it in `home/.chezmoitemplates/CLAUDE.md`. Verify the hash is stable across two renders and changes after touching a file in the plugin directory.
...
- [ ] 3.4 Port the guard to `plugins/claude-tooling/scripts/executable_claude-tooling-write-guard`. Make these changes:
  ...
  - replace the informational message with the substituted context file
  ...
  - exit 0 when `config.env` is missing
  - `Read` fast path under 50 ms (`time`)
- [ ] 3.5 Write `plugins/claude-tooling/context/claude-tooling.md` (condensed, 80 lines or fewer, `@@…@@` placeholders) covering every Content Coverage topic, including where hooks live. Verify with a checklist diff against the modified `claude-tooling-rule` spec topics, and confirm `grep -c '/Users/'` returns 0.
...
- [ ] 4.2 Delete `home/dot_local/bin/executable_claude-tooling-write-guard.tmpl` and `home/dot_claude/rules/claude-tooling.md.tmpl`. Create `home/.chezmoiremove` listing `.local/bin/claude-tooling-write-guard` and `.claude/rules/claude-tooling.md`. Verify `chezmoi managed | grep claude-tooling` shows only plugin paths.
...
- [ ] 4.4 Point the three `claude-tooling.md` references (`global-preferences.md.tmpl:37`, `check-claude-overrides.tmpl:47`, `clean-claude-orphans/SKILL.md.tmpl:26`) at the plugin's `context/claude-tooling.md`. Verify `grep -rn "rules/claude-tooling" home` returns nothing.
