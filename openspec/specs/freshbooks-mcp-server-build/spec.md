# Spec: freshbooks-mcp-server-build

## Purpose

Defines the automated post-fetch build step for the FreshBooks MCP server external dependency. The server ships as TypeScript source and must be compiled before any MCP client can launch it.
