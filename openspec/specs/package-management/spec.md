...
Any tag category in `packages.yaml` MAY declare an optional `brew_env` key containing a string-to-string map of environment variable names to values. These variables SHALL be exported into the shell environment before `brew bundle` runs, making them available to all Homebrew formula and cask installations in that bundle run.

#### Scenario: brew_env declared in an active category
- **WHEN** a tag category is active
...
- **AND** SHALL run before Claude plugins (position 39)
- **AND** Claude LaunchAgent (position 40) SHALL run last
...
The numbered script execution order (20, 21, 23, 24, 25, 26, 28) uses 10-point range grouping:
- **Categorical grouping**: Scripts in 20-29 range handle package management, 30-39 handle environment managers
- **Flexible spacing**: Within each range, scripts use 1-5 point spacing as needed (20, 21, 23, 24, 25, 26, 28)
