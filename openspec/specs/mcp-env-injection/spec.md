# Capability: MCP Env Injection

## Purpose

A lightweight wrapper script (`mcp-env-wrapper`) that sources per-server environment files before exec-ing MCP server commands. Provides consistent, launch-context-independent environment injection for MCP servers across both GUI clients (where env injection is necessary) and terminal clients (where it provides consistency).
